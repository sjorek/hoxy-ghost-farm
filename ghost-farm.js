...
Modifications by Stephan Jorek
Copyright (c) 2012
http://github.com/sjorek
*/
...
var projectName = 'Hoxy Ghost-Farm';
...
var opts = require('tav').set({
	...
	host: {
		note: 'Specify default farming server hostname.',
		value: 'localhost',
	},
	port: {
	...
	farm: {
		note: 'Hosts that '+projectName+' will act as a staging server for.',
		value: false,
	},
	'no-version-check': {
...
}, projectName + ", the web-hacking proxy.\nusage: node ghost-farm.js [--debug] [--rules=file] [--port=port] [--farm=…]");

var HTTP  = require('http');
var URL   = require('url');
var HTS   = require('hoxy/lib/http-transaction-state.js');
var Q     = require('hoxy/lib/asynch-queue.js');
var RULES = require('hoxy/lib/rules.js');
var RDB   = require('hoxy/lib/rules-db.js');

var proxyPort = Math.max(0, parseInt(opts.port)) || 8080;
var debug = opts.debug;

if (opts.farm) {
	opts.farm = (function(farm) {
		var source  = {},
		    sources = [],
		    target  = {},
		    targets = [];

		farm.split(',').map(function(sheep) {
			if (!/^[a-z0-9._-]+((:\d+)?:[a-z0-9._-]+(:\d+)?)?$/i.test(sheep)) {
				console.error('error: farm must be of the form /^(<localhost>(:<port>)?:)?<remotehost>(:<port>)?(,<another.localhost>(:<port>)?:<another.remotehost>(:<port>)?)*$/ exiting.');
				process.exit(1);
			}
			var parts  = sheep.split(':'),
			    local  = parts.shift(),
			    port   = Math.max(0, parseInt(parts[0])) ? ':' + parts.shift() : '',
			    remote = parts.join(':');
			if (!remote) {
				remote = '' + local;
				local  = opts.host + ':' + proxyPort;
			}
			local += port;
			if (typeof source[local] == 'undefined') {
				source[local] = remote;
				sources.push(local);
			} else {
				console.error('error: local farming sheep configured twice.', local);
				process.exit(1);
			}
			if (typeof target[remote] == 'undefined') {
				target[remote] = local;
				targets.push(remote);
			} else {
				console.error('error: remote farming sheep configured twice.', remote);
				process.exit(1);
			}
		});
		return {'source':source,'sources':sources,'target':target,'targets':targets};
	})(opts.farm);
}
...
if(useProxy = !!envProxy) {
	...
	console.log(projectName + ' using proxy '+envProxy);
}
...
HTTP.createServer(function handleRequest(request, response) {
	...
	if (/^\//.test(request.url) && opts.farm && request.headers.host &&
		typeof opts.farm.source[request.headers.host] !== 'undefined')
	{
		var sheep = opts.farm.source[request.headers.host];
		request.url = 'http://'+sheep+request.url;
		request.headers.host = sheep;
	}
		...
		reqPhaseRulesQ.execute(function(){
			...
			function sendResponse(respInfo) {
				...
				respPhaseRulesQ.execute(function(){
					...
					respInfo.headers['x-manipulated-by'] = projectName + ' on ' + opts.host + ':' + proxyPort;
...
}).listen(proxyPort, opts.host);

// done creating proxy
// #############################################################################
// print a nice info message

console.log(projectName+' running at '+opts.host+':'+proxyPort);
if (opts.farm) {
	console.log('staging mode is on. http://'+opts.host+':'+proxyPort+'/ will farm for:');
	for (var sheep in opts.farm.source) {
		console.log("\t", sheep, " <=> ", opts.farm.source[sheep]);
	}
}
